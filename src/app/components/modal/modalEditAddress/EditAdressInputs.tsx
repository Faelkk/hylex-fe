...
export default function AdressInputs({
  ...
  const { addNewAddress, addNewDefaultAddress, editAddress } = useAddress();
  ...
  const initialCepRef = useRef<string | undefined>(address?.cep);
  const prevCepRef = useRef<string | undefined>();
  ...
  useEffect(() => {
    ...
    if (
      cepValue &&
      cepValue.length === 8 &&
      cepValue !== initialCepRef.current &&
      cepValue !== prevCepRef.current
    ) {
      fetchAddressData(cepValue);
    }
    prevCepRef.current = cepValue;
  }, [cepValue, setValue]);
    ...
    <div className="p-5 flex flex-col">
      ...
      >
        ...
        <div className="flex flex-col-reverse md:flex-row justify-between gap-4">
          <button
            type="submit"
            className="bg-green-400 font-poppins p-1 md:max-w-[280px] w-full uppercase rounded-[5px] hover:bg-green-500 transition-colors drop-shadow-md text-gray-200"
          >
            Salvar alterações
          </button>
          <button
          ...
          >
            <a className="font-poppins uppercase font-medium text-black-300 ">
              {" "}
